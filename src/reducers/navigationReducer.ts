...
export const navigationReducer: Reducer<StateType, NavigationReducerAction> = (
	state: StateType,
	action: NavigationReducerAction
) => {
	switch (action.type) {
		case NAVIGATION_ACTIONS.TOGGLE_ACTIVE: {
			const whichNav = Object.keys(state).find(
				key => key === action.payload.replace("/", "")
			);
			if (whichNav) {
				const linkShouldBeActive =
					!state[whichNav as keyof typeof state].isActive;
				return {
					...INITIAL_STATE, // this resets the state, so no need looping and making inactive
					[whichNav]: {
						isActive: linkShouldBeActive,
						activeClasses: linkShouldBeActive
							? "text-LightPastelOrange"
							: "text-ashGray",
					},
				};
			}
